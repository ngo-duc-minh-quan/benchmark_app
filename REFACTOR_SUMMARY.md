...
| **Lọc dữ liệu rác** | Không lọc rác hoặc ngưỡng rộng `< 1000ms`. | Lọc chính xác các frame gián đoạn do hệ thống/background (`frameTime > 0 && frameTime < 250ms`). |
...
| **Độ chính xác pin** | Giữ nguyên số thập phân (`batteryDrain = Math.round(batteryDrain * 100) / 100`). | Hiển thị `Battery change <1%` khi chênh lệch pin dưới 1% để phản ánh đúng giới hạn cập nhật của OS API. |
...
| **Xử lý 60Hz Lock (Android)** | Tự động hạ `effectiveTargetHz` xuống 60 để nhân điểm GPU lên. | Chuyển thành `detectPossible60HzLock` (`boolean`). **Chỉ dùng hiển thị banner cảnh báo, KHÔNG nắn điểm GPU**. |
...
| Tính năng | Trước khi sửa | Sau khi sửa |
| :--- | :--- | :--- |
| **Đơn vị đo (Work Units)** | Single-Core cộng `primeSieve` count; Multi-Core cộng `100 ops` ngẫu nhiên (không cùng đơn vị). | **Đồng nhất Work Unit**: 1 Work Unit = `1x primeSieve(500,000) + 1x matMul(100)` cho cả Single-Core và Multi-Core Worker. |
...
| **Độ chuẩn xác đo lường** | Giới hạn hardcode `1,000,000 ops/sec`. | Log dữ liệu thô `workUnitsPerSecond = workUnits / (durationMs / 1000)` phục vụ calibrate theo thực nghiệm. |
